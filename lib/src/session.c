	...
	ChiakiLog *log)
{
	...
	session->connect_info.session_port = connect_info->session_port ? connect_info->session_port : SESSION_PORT;
	session->connect_info.stream_port = connect_info->stream_port ? connect_info->stream_port : 9296;
	session->connect_info.senkusha_port = connect_info->senkusha_port ? connect_info->senkusha_port : 9297;
...
static ChiakiErrorCode session_thread_request_session(ChiakiSession *session, ChiakiTarget *target_out)
{
	...
	else
	{
		for(struct addrinfo *ai=session->connect_info.host_addrinfos; ai; ai=ai->ai_next)
		{
			...
			set_port(sa, htons(session->connect_info.session_port ? session->connect_info.session_port : SESSION_PORT));
